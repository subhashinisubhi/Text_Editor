...
public class TextEditor extends Frame {
  ...
  int fsize = 19;
  UndoableTextArea txt = new UndoableTextArea();
  String filename = "Untitled", st, fn = "untitled", dn;
  ...
  class Open implements ActionListener {
    /*
     * public void actionPerformed(ActionEvent ae) { FileDialog fd=new
     * FileDialog(TextEditor.this,"Select File",FileDialog.LOAD); fd.show();
     * if((fn=fd.getFile())!=null) { filename=fd.getDirectory()+fd.getFile();
     * dn=fd.getDirectory(); setTitle(filename+" - TextEditor"); readFile(); }
     * txt.requestFocus(); }
     */
    public void actionPerformed(ActionEvent ope) {
      FileDialog fDialog = new FileDialog(TextEditor.this, "Open", FileDialog.LOAD);
      fDialog.show();
      if (fDialog.getFile() != null) {
        filename = fDialog.getDirectory() + fDialog.getFile();
        setTitle(fDialog.getFile() + " - TextEditor");
        ReadFile();
      }
      txt.requestFocus();
    }
  ...
  class Save implements ActionListener {
    public void actionPerformed(ActionEvent ae) {
      FileDialog fDialog = new FileDialog(TextEditor.this, "Save", FileDialog.SAVE);
      fDialog.show();
      if (fDialog.getFile() != null) {
        filename = fDialog.getDirectory() + fDialog.getFile();
        setTitle(fDialog.getFile() + " - TextEditor");
  ...
  class ConfirmDialog extends JPanel {
    public ConfirmDialog() {
      ...
      result = JOptionPane.showConfirmDialog(this,
          " Do you want to save changes to\n" + filename/* ,"Message Box",JOptionPane.QUESTION_MESSAGE */);
